...
private class GithubRequestExecutorImpl @Inject()(val wsClient: WSClient) extends RequestsExecutor with GithubRequestExecutor{
  override val state: RequestsExecutorState = {
    ...
    if (ipResponse.status == 200) {
      ...
      new RequestsExecutorState(ip, RateLimit(60, 60, 0))
    }else {
